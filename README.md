...
Поля:
- `productsList: string[]` - Геттер. Возвращает список id продуктов в заказе.
...
- `total: number` - Геттер и сеттер. Возвращает и перезаписывает сумму товаров в корзине

Методы:
- `addProduct(id: string)` - Добавляет товар в заказ.
